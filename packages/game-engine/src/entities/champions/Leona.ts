...
export class Leona extends ChessObject {
  skill(): void {
    ...
    for (const enemy of top3NearestEnemies) {
      ...
      this.applyDebuff(enemy, {
        ...
        name: 'Sunlight',
        description:
          'Marked by Sunlight for 3 turns. When an ally deals damage, consumes mark for bonus magic damage.',
        duration: 3,
        maxDuration: 3,
  ...
  private getTop3NearestEnemies(): ChessObject[] {
    const enemies = this.game.board.filter(
      (p) =>
        p.stats.hp > 0 &&
        p.blue !== this.chess.blue &&
        Math.abs(p.position.x - this.chess.position.x) <= 2 &&
        Math.abs(p.position.y - this.chess.position.y) <= 2,
    );
