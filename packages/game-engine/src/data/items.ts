import { Chess, ChessStats } from '../types';

/**
 * Context object for item condition evaluation
 * This is passed to condition functions without coupling to ChessObject class
 */
export interface ItemConditionContext {
  chess: Chess;
  maxHp: number;
}
...
export interface ItemEffect {
  ...
  value: number;
  type: 'add' | 'multiply';
  condition?: (ctx: ItemConditionContext) => boolean;
}
...
export const combinedItems: ItemData[] = [
  ...
  },
  {
    id: 'giant_slayer',
    name: 'Giant Slayer',
    description: '+15% Damage Amplification against chess having more than 200 HP.',
    cost: 0,
  ...
  // Recurve Bow Combinations
  {
    ...
    effects: [{ stat: 'sunder', value: 30, type: 'add' }],
    isBasic: false,
  ...
  },
  {
    ...
    description: 'When attack, the user can make another attack but it deals 50% less damage.',
    cost: 0,
  ...
  },
  {
    ...
    description: 'Gains 12% Durability. Convert 25% of damage taken into AD and AP for 3 turns.',
    cost: 0,
  ...
  },
  {
    id: 'wit_s_end',
    name: "Wit's End",
    description: 'Each attacks deals (5 + 25% of bonus AD) magic damage to the target.',
    cost: 0,
  ...
  },
  {
    ...
    effects: [
      ...
      { stat: 'magicResistance', value: 25, type: 'add' },
      {
        ...
        value: 20,
        type: 'add',
        condition: (ctx) => ctx.chess.stats.hp < ctx.maxHp * 0.6,
      },
      {
  ...
  },
  {
    ...
    description: 'Gain 10% Durability. While above 50% Health, instead gain 20% Durability.',
    cost: 0,
    ...
    effects: [
      ...
      { stat: 'durability', value: 10, type: 'add' },
      {
        ...
        value: 10,
        type: 'add',
        condition: (ctx) => ctx.chess.stats.hp > ctx.maxHp * 0.5,
      },
    ],
    isBasic: false,
  ...
  },
  {
    id: 'evenshroud',
    name: 'Evenshroud',
    description: 'Reduces Physical and Magic Resistance of adjacent enemies by 10.',
    cost: 0,
  ...
  },
  {
    id: 'spirit_visage',
    name: 'Spirit Visage',
    description: 'Grants 30% health heal from all sources. Restore 5% of missing Health each turn.',
    cost: 0,
...
export const allItems: ItemData[] = [...basicItems, ...combinedItems, ...viktorModules];
...
export function findCombinedItem(item1Id: string, item2Id: string): ItemData | undefined {
  return combinedItems.find((item) => {
    if (!item.recipe) return false;
    const [r1, r2] = item.recipe;
    return (r1 === item1Id && r2 === item2Id) || (r1 === item2Id && r2 === item1Id);
  });
}
