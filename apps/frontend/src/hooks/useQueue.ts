...
import { setQueueStatus, joinQueue as joinQueueAction, leaveQueue, setMatchFound } from '../store/gameSlice'
...
export const useQueue = () => {
  ...
  useEffect(() => {
    ...
    socket.on('queueJoined', (data: { position: number; playersInQueue: number }) => {
    ...
    socket.on('queueStatus', (data: { position: number | null; queueSize: number; inQueue: boolean }) => {
      dispatch(setQueueStatus({
        position: data.position,
    ...
    socket.on('matchFound', (data: { game: any; opponent: { id: string; username: string }; phase?: string }) => {
