...
const ChessPieceComponent = styled(motion.div)<{
  isBlue: boolean;
  isNeutral: boolean;
  canSelect: boolean;
  isAttacking?: boolean;
  ...
  border-radius: 8px;
  border: 1px solid
    ${(props) => (props.isNeutral ? '#9333ea' : props.isBlue ? '#3b82f6' : '#ef4444')};
  background: ${(props) =>
    props.isNeutral
  ...
  box-shadow: ${(props) =>
    props.hasShield
      ? '0 0 8px 3px rgba(255, 255, 255, 0.6), 0 0 2px 2px ' +
        (props.isNeutral ? '#9333ea' : props.isBlue ? '#3b82f6' : '#ef4444')
...
const SkillIcon = styled.div<{ isActive: boolean; onCooldown: boolean; currentCooldown: number }>`
  ...
  height: 24px;
  border-radius: 4px;
  border: ${(props) => (props.isActive ? '2px solid #ffd700' : '2px solid #9333ea')};
  background: ${(props) =>
    props.onCooldown
      ? 'rgba(0, 0, 0, 0.7)'
      : props.isActive
...
const SummonerSpellIcon = styled.div<{
  $spellType: string;
  $onCooldown: boolean;
  ...
  border: 2px solid
    ${(props) =>
      props.$onCooldown ? '#666' : SUMMONER_SPELL_COLORS[props.$spellType] || '#87CEEB'};
  background: ${(props) =>
    props.$onCooldown
      ? 'rgba(0, 0, 0, 0.7)'
  ...
  &::after {
    content: ${(props) =>
      props.$onCooldown && props.$currentCooldown > 0
        ? `'${Math.ceil(props.$currentCooldown)}'`
        : '""'};
    position: absolute;
    top: 50%;
    left: 50%;
...
const ItemIcon = styled.div<{ $onCooldown?: boolean; $currentCooldown?: number }>`
  ...
  &::after {
    content: ${(props) =>
      props.$onCooldown && props.$currentCooldown
        ? `'${Math.ceil(props.$currentCooldown)}'`
        : '""'};
    position: absolute;
    top: 50%;
...
export const ChessPieceRenderer: React.FC<{
  ...
  const piecePurchaseAnimations = itemPurchaseAnimations.filter(
    (anim) => anim.targetId === piece.id,
  );
  ...
  const hasShield =
    piece.shields &&
    piece.shields.length > 0 &&
    piece.shields.reduce((sum, s) => sum + s.amount, 0) > 0;
  ...
  return (
    ...
    >
          ...
          <img
            ...
            onError={(e) => {
              ...
              if (parent && piece.summonerSpell) {
                ...
                parent.innerHTML =
                  typeof spellType === 'string' ? spellType.charAt(0).toUpperCase() : '?';
            ...
            <ItemPurchaseEffect key={purchaseAnim.id}>
              ...
              {itemData && (
                <GoldSpentText
                  ...
                  transition={{ duration: 0.5, ease: 'easeOut' }}
                >
                  <img src="/icons/dollar.png" alt="Gold" width={12} height={12} />-{itemData.cost}
                </GoldSpentText>
              )}
            </ItemPurchaseEffect>
          );
      ...
      {isAttacking && attackAnimation?.guinsooProc && (
        ...
        >
          <div className="piece-icon">
            <img src={imageUrl} alt={`${piece.name} afterimage`} />
          </div>
        </AfterimageEffect>
      )}
    </ChessPieceComponent>
