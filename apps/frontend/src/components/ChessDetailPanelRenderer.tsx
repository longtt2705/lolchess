...
import {
  ...
  getIconConfig,
  getImageUrl,
  getSkillStateDisplay,
  getStatIcon,
  isChampion,
  VIKTOR_DAMAGE_THRESHOLDS,
...
export const ChessDetailPanelRenderer: React.FC<{
  ...
  const handleItemHover = (e: React.MouseEvent, itemId: string) => {
    ...
    const left =
      rect.right + 10 + tooltipWidth > viewportWidth
  ...
  const checkItemNotPurchasability = (item: ItemData, piece: ChessPiece, gold: number) => {
    ...
    if (itemLength === 3 && (piece as any).items.every((item: ItemData) => !isBasicItem(item.id)))
      return true;
  ...
  const displayDetailStats = (detailViewPiece: any, statName: string): string => {
    const base =
      statName === 'attackRange'
        ? (detailViewPiece.rawStats as any).attackRange?.range || 0
        : (detailViewPiece.rawStats as any)[statName] || 0;
    const current =
      statName === 'attackRange'
        ? detailViewPiece.stats.attackRange?.range || 0
        : detailViewPiece.stats[statName] || 0;
      ...
      {detailViewPiece ? (
        <>
          ...
          <div className="chess-header">
            <div
              className={`chess-icon ${detailViewPiece.ownerId === 'neutral' ? 'neutral' : detailViewPiece.blue ? 'blue' : 'red'}`}
            >
              <img
            ...
            <div className="chess-info">
              ...
              <div className="chess-position">
                Position: ({detailViewPiece.position.x}, {detailViewPiece.position.y}) • Owner:{' '}
                {detailViewPiece.ownerId === 'neutral'
                  ? 'Neutral'
          ...
          <div className="hp-bar-section">
            <div className="hp-label">
              ...
              <span className="hp-numbers">
                {formatNumber(detailViewPiece.stats.hp)} /{' '}
                {formatNumber(detailViewPiece.stats.maxHp)}
                {detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.maxHp !== detailViewPiece.stats.maxHp && (
                    <span
                      style={{
                        fontSize: '10px',
                        color: 'var(--secondary-text)',
                ...
                <div className="hp-label" style={{ marginTop: '12px' }}>
                  ...
                  <span className="hp-numbers" style={{ color: 'rgba(255, 255, 255, 0.9)' }}>
                    ...
                    {detailViewPiece.shields.length > 0 &&
                      detailViewPiece.shields[0].duration !== Number.MAX_SAFE_INTEGER && (
                        <span
                          style={{
                            fontSize: '10px',
                            color: 'var(--secondary-text)',
                        ...
                        >
                          ({detailViewPiece.shields[0].duration}{' '}
                          {detailViewPiece.shields[0].duration === 1 ? 'turn' : 'turns'})
                        </span>
                      )}
                  </span>
                </div>
                <div className="hp-bar reverse">
                  <div
                    ...
                    style={{
                      ...
                      background:
                        'linear-gradient(90deg, rgba(255, 255, 255, 0.9) 0%, rgba(200, 200, 220, 0.9) 50%, rgba(180, 180, 200, 0.9) 100%)',
            ...
            <div className="hp-bar">
              <div
                className="hp-fill"
                style={{
                  width: `${(detailViewPiece.stats.hp / detailViewPiece.stats.maxHp) * 100}%`,
                }}
              />
            </div>
          </div>
          ...
          <div className="stats-grid">
            <div
              className="stat-item"
              onMouseEnter={(e) =>
                handleTooltipShow(e, `Attack Damage ${displayDetailStats(detailViewPiece, 'ad')}`)
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/AD.svg" alt="Attack" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.ad !== detailViewPiece.stats.ad
            ...
            <div
              className="stat-item"
              onMouseEnter={(e) =>
                handleTooltipShow(e, `Ability Power ${displayDetailStats(detailViewPiece, 'ap')}`)
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/AP.svg" alt="Ability" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.ap !== detailViewPiece.stats.ap
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Physical Resistance (${damageReductionPercentage(detailViewPiece.stats.physicalResistance)}%) ${displayDetailStats(detailViewPiece, 'physicalResistance')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/Armor.svg" alt="Physical Resistance" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.physicalResistance !==
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Magic Resistance (${damageReductionPercentage(detailViewPiece.stats.magicResistance)}%) ${displayDetailStats(detailViewPiece, 'magicResistance')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/MagicResist.svg" alt="Magic Resistance" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.magicResistance !== detailViewPiece.stats.magicResistance
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Movement Speed ${displayDetailStats(detailViewPiece, 'speed')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/speed.png" alt="Speed" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.speed !== detailViewPiece.stats.speed
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Attack Range ${displayDetailStats(detailViewPiece, 'attackRange')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/Range.svg" alt="Range" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.attackRange.range !==
              ...
              <AttackRangeIndicator attackRange={detailViewPiece.stats.attackRange} size={20} />
            </div>
            <div
              className="stat-item"
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Sunder (Armor Penetration) ${displayDetailStats(detailViewPiece, 'sunder')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/AS.svg" alt="Sunder" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  (detailViewPiece as any).rawStats &&
                  (detailViewPiece as any).rawStats.sunder !== (detailViewPiece as any).stats.sunder
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Critical Strike Chance (%) ${displayDetailStats(detailViewPiece, 'criticalChance')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/CritChance.svg" alt="Critical Chance" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  (detailViewPiece as any).rawStats &&
                  (detailViewPiece as any).rawStats.criticalChance !==
                    (detailViewPiece as any).stats.criticalChance
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Critical Strike Damage (%) ${displayDetailStats(detailViewPiece, 'criticalDamage')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/CritDamage.svg" alt="Critical Damage" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  (detailViewPiece as any).rawStats &&
                  (detailViewPiece as any).rawStats.criticalDamage !==
                    (detailViewPiece as any).stats.criticalDamage
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Damage Amplification (%) ${displayDetailStats(detailViewPiece, 'damageAmplification')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/icon-da.png" alt="Damage Amplification" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  (detailViewPiece as any).rawStats &&
                  (detailViewPiece as any).rawStats.damageAmplification !==
                    (detailViewPiece as any).stats.damageAmplification
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Cooldown Reduction (%) ${displayDetailStats(detailViewPiece, 'cooldownReduction')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/icon-cdr.webp" alt="Cooldown Reduction" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  (detailViewPiece as any).rawStats &&
                  (detailViewPiece as any).rawStats.cooldownReduction !==
                    (detailViewPiece as any).stats.cooldownReduction
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Lifesteal (%) ${displayDetailStats(detailViewPiece, 'lifesteal')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/icon-sv.png" alt="Lifesteal" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  (detailViewPiece as any).rawStats &&
                  (detailViewPiece as any).rawStats.lifesteal !==
                    (detailViewPiece as any).stats.lifesteal
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `HP Regeneration per Turn ${displayDetailStats(detailViewPiece, 'hpRegen')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/icon-hp-regen.png" alt="HP Regen" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  detailViewPiece.rawStats.hpRegen !== detailViewPiece.stats.hpRegen
            ...
            <div
              ...
              onMouseEnter={(e) =>
                handleTooltipShow(
                  e,
                  `Durability (% damage reduction) ${displayDetailStats(detailViewPiece, 'durability')}`,
                )
              }
              onMouseLeave={handleTooltipHide}
            >
              <span className="stat-label">
                <img src="/icons/icon-durability.png" alt="Durability" width={14} height={14} />
              </span>
              <span
                className={`stat-value ${
                  detailViewPiece.rawStats &&
                  +((detailViewPiece.rawStats as any).durability || 0) !==
              ...
              <div className="skill-card">
                <div className="skill-card-header">
                  <div style={{ position: 'relative' }}>
                    <img
                      src={`/icons/${detailViewPiece.name.toLowerCase()}_skill.webp`}
                      alt={detailViewPiece.name}
                  ...
                  <div className="skill-info">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {detailViewPiece.skill.type === 'active' &&
                        detailViewPiece.skill.targetTypes !== 'none' &&
                        detailViewPiece.skill.attackRange?.range && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <img
                              src={getStatIcon('attackRange')}
                              alt="Range"
                              width={14}
                              height={14}
                            />
                      ...
                      {detailViewPiece.skill.type === 'active' &&
                        detailViewPiece.skill.targetTypes !== 'none' &&
                        detailViewPiece.skill.attackRange && (
                          <AttackRangeIndicator
                            attackRange={detailViewPiece.skill.attackRange}
                            size={16}
                          />
                        )}
                      {detailViewPiece.skill.cooldown > 0 && (
                        <div>
                          <img
                            src={getStatIcon('cooldownReduction')}
                            alt="Cooldown"
                            width={14}
                            height={14}
                          />
                          <span style={{ color: 'var(--gold)', marginLeft: '4px' }}>
                            {detailViewPiece.skill.cooldown}
                          </span>
                        </div>
                      )}
                    </div>
                    <div
                      className={`card-cooldown ${detailViewPiece.skill.currentCooldown > 0 ? 'cooling' : 'ready'}`}
                    >
          ...
          {detailViewPiece.summonerSpell && (
            ...
            >
              <div
                ...
                style={{ color: getSummonerSpellColor(detailViewPiece.summonerSpell.type) }}
              >
                  ...
                  <div className="skill-info">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div>
                        ...
                        />
                        <span style={{ color: 'var(--gold)', marginLeft: '4px' }}>
                          {detailViewPiece.summonerSpell.cooldown}
                        </span>
                      </div>
                    </div>
                    <div
                      className={`card-cooldown ${detailViewPiece.summonerSpell?.currentCooldown > 0 ? 'cooling' : 'ready'}`}
                    >
          ...
          <div className="debuff-section">
            ...
            {detailViewPiece.debuffs && detailViewPiece.debuffs.length > 0 ? (
              detailViewPiece.debuffs.map((debuff: any, index: number) => {
                ...
                const debuffClass = isTransformation
                  ? 'transform-debuff'
                  : isAura
                    ? 'aura-debuff'
                    : isBuff
                ...
                return (
                  <div key={index} className={`debuff-card ${debuffClass}`}>
                    <div className="debuff-card-header">
                      <div className="debuff-icon">
                        ...
                        <div
                          ...
                          style={{ display: debuff.casterName ? 'none' : 'block' }}
                        >
                      ...
                      <div className="debuff-info">
                        ...
                        {isAura && (
                          <div
                            className={`card-duration ${isAura ? 'active' : debuff.duration <= 1 ? 'cooling' : 'ready'}`}
                          >
                            Active (Aura)
                          </div>
                        )}
                    ...
                    {debuff.effects && debuff.effects.length > 0 && (
                      ...
                      >
                        {debuff.effects
                          .filter((effect: any) => effect.modifier !== 0)
                          .map((effect: any, idx: number) => (
                            ...
                            >
                              ...
                              />
                              <span
                                style={{ color: effect.modifier > 0 ? 'var(--gold)' : '#ef4444' }}
                              >
                                {effect.modifier > 0 ? '+' : ''}
                                {effect.type === 'percentAdd' ? effect.modifier : effect.modifier}
                                {effect.type === 'percentAdd' ? '%' : ''}
                              </span>
                            </div>
                          ))}
                      </div>
              ...
              <div className="items-grid">
                {(detailViewPiece as any).items.map((item: any, index: number) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`item-card equipped ${item.unique ? 'unique' : ''}`}
                    >
                      <div
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          <div className="card-name" style={{ marginBottom: '8px' }}>
                            {item.name}
                          </div>
                          ...
                          {itemData?.cooldown && itemData.cooldown > 0 && (
                            <div
                              className={`card-cooldown ${item.currentCooldown > 0 ? 'cooling' : 'ready'}`}
                                  ...
                                  >
                                    ...
                                    <span style={{ color: 'var(--gold)' }}>
                                      ...
                                      {effect.type === 'multiply'
                                        ? Math.floor(effect.value * 100 - 100)
                                        : effect.value}
                                      {effect.type === 'multiply' ? '%' : ''}
                                    </span>
                                  </div>
          ...
          {isChampion(detailViewPiece) &&
            championData?.items_suggestions &&
            championData.items_suggestions.length > 0 && (
              <div
              ...
              >
                ...
                <div
                  style={{ fontSize: '11px', color: 'var(--secondary-text)', marginBottom: '12px' }}
                >
                ...
                <SuggestedItemsGrid>
                  {championData.items_suggestions.slice(0, 8).map((itemId, index) => {
                    ...
                    return (
                      ...
                      >
                        {index < 3 && <div className="priority-badge">{index + 1}</div>}
                        {itemData.icon ? (
              ...
              return (
                ...
                >
                  ...
                  {itemData.description && (
                    <div className="tooltip-description">{itemData.description}</div>
                  )}
                  ...
                  {itemData.recipe && (
                    <div className="tooltip-recipe">
                      ...
                      →{itemData.icon && <img src={itemData.icon} alt="" />}
                    </div>
                  )}
                </ItemTooltip>
              );
          ...
          {detailViewPiece.name === 'Viktor' &&
            ...
            (() => {
              ...
              const purchasedModules =
                detailViewPiece.items?.filter((item) =>
                  viktorModules.some((m) => m.id === item.id),
                ) || [];
              ...
              return (
                <div className="shop-section" style={{ borderColor: 'rgba(147, 51, 234, 0.5)' }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    ...
                    <div
                      style={{ fontSize: '10px', color: 'var(--secondary-text)', marginTop: '2px' }}
                    >
                      {moduleCount >= 3
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                        ...
                        >
                          <img
                            ...
                            style={{
                              ...
                              height: '36px',
                              objectFit: 'contain',
                              filter: isModuleUnlocked ? 'none' : 'grayscale(100%)',
                            }}
                            onError={(e) => {
                              e.currentTarget.style.display = 'none';
                            }}
                        ...
                        <div style={{ flex: 1 }}>
                          <div
                            style={{
                              color: isModuleUnlocked ? '#a855f7' : 'var(--secondary-text)',
                              fontWeight: 'bold',
                              fontSize: '13px',
                              marginBottom: '4px',
                            }}
                      ...
                      >
                        ...
                        <span>
                          {isModuleUnlocked
                            ? 'Acquire Module'
                            : `Unlock at ${nextThreshold} damage`}
                        </span>
                      </button>
                      {!isMyTurn && (
                        <div
          ...
          {detailViewPiece.ownerId === currentPlayer?.userId && isChampion(detailViewPiece) && (
            <div className="shop-section">
              ...
              {itemsLoading ? (
                <div
                  style={{ textAlign: 'center', padding: '20px', color: 'var(--secondary-text)' }}
                >
                  Loading items...
                </div>
                  ...
                  {shopItemsData.map((item: ItemData) => (
                    ...
                    >
                      <button
                        ...
                        disabled={checkItemNotPurchasability(
                          item,
                          detailViewPiece,
                      ...
                      <div className="shop-item-tooltip">
                        ...
                        <div className="tooltip-cost">
                          <Coins size={12} />{' '}
                          {item.isBasic ? getCurrentItemPrice(currentPlayer) : item.cost} gold
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <>
          ...
          <div className="no-selection">
            ...
            <div className="placeholder-hint">
              Click on any chess piece to view detailed information
            </div>
          </div>
        </>
      )}
