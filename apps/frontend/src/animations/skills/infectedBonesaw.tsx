...
const InfectedBonesawAnimation: React.FC<SkillAnimationConfig> = ({
  ...
  if (!targetPosition || !targetId) {
    ...
    console.warn('[InfectedBonesaw] Missing targetPosition or targetId:', {
      targetPosition,
      targetId,
    });
    return null;
    ...
    <>
      ...
      <ImpactEffect
        style={{
          ...
          pointerEvents: 'none',
          background:
            'radial-gradient(circle, rgba(255, 100, 100, 0.9) 0%, rgba(139, 69, 19, 0.6) 50%, transparent 100%)',
        }}
      ...
      {showParticles && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <ParticleEffectComponent position={targetPixels} count={10} duration={500} />
        </motion.div>
      )}
