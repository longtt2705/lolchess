    ...
    <>
      ...
      {phase === 'swirl' && (
        <motion.div
          style={{
            ...
            height: 4,
            background:
              'linear-gradient(90deg, rgba(255, 215, 0, 0.8), rgba(218, 165, 32, 0.6), rgba(255, 215, 0, 0.4))',
            transformOrigin: '0 50%',
            transform: `rotate(${
              Math.atan2(targetPixels.y - casterPixels.y, targetPixels.x - casterPixels.x) *
              (180 / Math.PI)
            }deg)`,
            borderRadius: 2,
            pointerEvents: 'none',
      ...
      {showParticles && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <ParticleEffectComponent position={targetPixels} count={16} duration={500} />
        </motion.div>
      )}
    </>
  );
