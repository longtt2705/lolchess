...
const LightningSpark = styled(motion.div)`
  ...
  height: 16px;
  border-radius: 50%;
  background: radial-gradient(circle, ${ENERGY_CORE} 0%, ${HEXTECH_BLUE} 50%, transparent 100%);
  pointer-events: none;
  z-index: 104;
...
const SiphonPowerAnimation: React.FC<SkillAnimationConfig> = ({
  ...
  return (
    <>
      ...
      {showLightning &&
        targetPixels &&
        aoeTargets.map((aoeTarget, index) => {
      ...
      {showImpact && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <ParticleEffectComponent position={casterPixels} count={8} duration={400} />
        </motion.div>
      )}
    </>
  );
...
export const siphonPowerRenderer: SkillAnimationRenderer = {
  ...
  duration: (config: SkillAnimationConfig) => {
    const hasAoeTargets =
      config.viktorModules?.aoeTargets && config.viktorModules.aoeTargets.length > 0;
