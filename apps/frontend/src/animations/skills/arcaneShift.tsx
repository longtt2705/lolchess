...
const ArcaneOrb = styled(motion.div)`
  ...
  background: radial-gradient(
    circle,
    rgba(5, 150, 170, 1) 0%,
    rgba(5, 150, 170, 0.8) 50%,
    rgba(5, 150, 170, 0.4) 100%
  );
  box-shadow:
...
const TeleportFlash = styled(motion.div)`
  ...
  background: radial-gradient(
    circle,
    rgba(5, 150, 170, 0.7) 0%,
    rgba(5, 150, 170, 0.4) 40%,
    transparent 70%
  );
  pointer-events: none;
  z-index: 97;
    ...
    <>
      ...
      {showArrival && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <ParticleEffectComponent position={targetPixels} count={20} duration={600} />
        </motion.div>
      )}
    </>
  );
