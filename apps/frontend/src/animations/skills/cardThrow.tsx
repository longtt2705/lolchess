...
const CardProjectile = styled(motion.div)<CardProjectileProps>`
  ...
  box-shadow:
    0 0 12px ${(props) => props.color}AA,
    0 0 24px ${(props) => props.color}66;
  display: flex;
  align-items: center;
...
const CardThrowAnimation: React.FC<SkillAnimationConfig> = ({
  ...
  const [impacts, setImpacts] = useState<
    { id: number; position: { x: number; y: number }; color: string }[]
  >([]);
  ...
  useEffect(() => {
    if (activeCardIndex < totalCardCount) {
      const timer = setTimeout(
        () => {
          setCards((prev) =>
            prev.map((card, idx) => (idx === activeCardIndex ? { ...card, launched: true } : card)),
          );
    ...
    <>
      ...
      {particles.map((particle) => (
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <ParticleEffectComponent position={particle.position} count={8} duration={500} />
        </motion.div>
      ))}
    </>
  );
