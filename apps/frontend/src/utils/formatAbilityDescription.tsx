...
function parseScalingExpression(expr: string): ParsedExpression | null {
  ...
  for (const segment of segments) {
    ...
    const armorMatch = trimmed.match(
      /([+-]?\s*[\d.]+)\s*%\s*(?:of\s+)?(?:Physical\s+Resistance|Armor)/i,
    );
          ...
          <span key={index} style={{ color: STAT_COLORS.ap, fontWeight: 600 }}>
            ...
            <img
              ...
              style={{
                ...
                marginLeft: '2px',
                filter:
                  'brightness(0) saturate(100%) invert(78%) sepia(47%) saturate(1000%) hue-rotate(130deg) brightness(95%) contrast(95%)',
              }}
            />
          </span>,
        );
          ...
          <span key={index} style={{ color: STAT_COLORS.ad, fontWeight: 600 }}>
            ...
            <img
              ...
              style={{
                ...
                marginLeft: '2px',
                filter:
                  'brightness(0) saturate(100%) invert(55%) sepia(95%) saturate(1000%) hue-rotate(10deg) brightness(100%) contrast(100%)',
              }}
            />
          </span>,
        );
 ...
 */
export function formatAbilityDescription(description: string, stats: ChessStats): React.ReactNode {
  ...
  const scalingPattern =
    /\([\d\s+\-*/%]+(?:of\s+)?(?:AP|AD|bonus\s+AD|max\s+HP|HP|health|Physical\s+Resistance|Armor|Magic\s+Resistance|MR|Sunder)(?:[\s+\-*/%\d]+(?:of\s+)?(?:AP|AD|bonus\s+AD|max\s+HP|HP|health|Physical\s+Resistance|Armor|Magic\s+Resistance|MR|Sunder))*\)/gi;
  ...
  while ((match = scalingPattern.exec(description)) !== null) {
    ...
    if (match.index > lastIndex) {
      ...
      parts.push(<span key={keyIndex++}>{replaceMoveSpeedWithIcon(textBeforeMatch)}</span>);
    }
    ...
    if (parsed) {
      ...
      parts.push(
        ...
        >
          <span style={{ color: '#FFFFFF', fontWeight: 700 }}>{calculatedValue}</span>
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9em' }}>{' = ('}</span>
          {formula}
          <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9em' }}>{')'}</span>
        </span>,
      );
  ...
  if (lastIndex < description.length) {
    ...
    parts.push(<span key={keyIndex++}>{replaceMoveSpeedWithIcon(remainingText)}</span>);
  }
