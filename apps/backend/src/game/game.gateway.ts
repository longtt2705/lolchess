  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit, OnModuleDestroy
{
   ...
   */
  afterInit() {
    this.logger.log('GameGateway initialized, subscribing to bot action results');
  ...
  private handleBotActionResult(result: BotActionResult): void {
    const { gameId, botPlayerId, success, game, oldGame, message, isGameOver, winner } = result;

    this.logger.log(`Received bot action result for game ${gameId}, success: ${success}`);
  ...
  async handleResign(@MessageBody() data: { gameId: string }, @ConnectedSocket() client: Socket) {
    try {
      const { gameId } = data;
      const userId = this.socketUsers.get(client.id);
  ...
  private async checkAndQueueBotTurn(gameId: string, game: any): Promise<void> {
    ...
    this.logger.log(`Bot turn detected for ${currentPlayerId} in game ${gameId}, queuing action`);
